...
- Extraction is cost-bounded: a `--start`/`--end` range uploads only that window, and the extracted audio is hard-capped in bytes and upload-chunk count regardless, so a long or oversized input can't run up unbounded API cost.
...
- Runs `ffmpeg` / `ffprobe` locally to extract frames as JPEGs and, when Whisper is needed, a mono 16 kHz audio clip. Both frame engines span the whole video (the tail is never silently dropped) but their transient candidate JPEGs are bounded by a disk watchdog that aborts a cut-heavy, all-keyframe, or hostile input before it fills the disk — the `efficient` keyframe pass gets the same watchdog as scene detection, so an I-frame-per-frame video can't materialize the whole stream before the frame cap thins it. Fetched captions are parsed with a bounded read and cue-count ceiling, so an oversized/hostile subtitle response can't balloon memory
- Bounds Whisper cost: when a `--start`/`--end` range is set, only that window's audio is extracted and billed (not the whole video trimmed afterwards); regardless of range, the extracted audio is hard-capped in bytes and upload-chunk count before anything is sent, so a long low-bitrate remote video (within the download cap) or a large local file can't run up unbounded transcription API cost — it aborts with guidance to narrow via `--start`/`--end`
...
- Network egress is limited to (a) fetching the URL you pass, via yt-dlp (optionally through a trusted `WATCH_YTDLP_PROXY`), and (b) uploading extracted audio to a Whisper API only when you've opted in with a key. No other outbound calls.
